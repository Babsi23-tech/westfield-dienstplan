...
let adminGesamtKwNeu = 1;

function installiereAdminGesamtDienstplanNeu() {
  installiereAdminAnsichtNeu();
  ...
  panel.innerHTML = `
    ...
    <p style="color:#666;">
      Dienstplan aller Mitarbeitenden.
    </p>
    ...
    <div id="adminGesamtDienstplanListe">
      <div class="empty-state">
...
async function ladeAdminGesamtDienstplanNeu() {
  installiereAdminGesamtDienstplanNeu();

  const liste =
    document.getElementById(
      'adminGesamtDienstplanListe'
    );
  ...
  try {
    ...
    rendereAdminGesamtDienstplanNeu();
...
function adminGesamtVerfuegbareKwsNeu() {
  return [
    ...new Set(
      (adminGesamtDienstplanNeu || [])
        .map(function(z) {
          return Number(z.kw || 0);
        })
        .filter(function(kw) {
          return kw > 0;
        })
...
function wechselAdminGesamtKwNeu(richtung) {
  const kws =
    adminGesamtVerfuegbareKwsNeu();
...
function adminDienstZeileNeu(
  ...
  return `
    <div
      ...
      "
    >
      <strong>${escapeHtmlNeu(titel)}:</strong>
      ${escapeHtmlNeu(name)}
      ${
        zusatz
          ? '<div style="font-size:12px;color:#666;margin-top:2px;">' +
            escapeHtmlNeu(zusatz) +
            '</div>'
          : ''
      }
    </div>
...
function rendereAdminGesamtDienstplanNeu() {
  const liste =
    document.getElementById(
      'adminGesamtDienstplanListe'
    );
  ...
  if (!tage.length) {
    liste.innerHTML =
      '<div class="empty-state">Für diese KW gibt es keine Einträge.</div>';
    return;
  }
...
// Bestehendes Admin-Laden erweitern, ohne die vorhandenen
// Tausch-, Wunsch- und PIN-Funktionen zu verändern.
const ladeAdminAnfragenOhneGesamtplanNeu =
...
ladeAdminAnfragenNeu =
  async function() {
    ...
    await Promise.all([
      ladeAdminAnfragenOhneGesamtplanNeu(),
      ladeAdminGesamtDienstplanNeu()
    ]);
  };
